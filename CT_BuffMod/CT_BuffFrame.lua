...
function CT_BuffButton_Update()
	...
	local debuffSlot = getglobal(name.."Border");
	local debuffType = GetPlayerBuffDispelType(GetPlayerBuff(this:GetID(), "HELPFUL|HARMFUL"));
...
function CT_BuffButton_OnLoad()
	...
	if ( CT_BuffIsDebuff( temp ) == 1 ) then
		...
		getglobal(this:GetName() .. "Border"):Show();
	else
		getglobal(this:GetName() .. "DescribeText"):SetTextColor(1, 1, 0);
